...
# ----------------------------------------------------------------
# NEURAL NETWORK CLASS WITH L2 REG, MULTIPLE OPTIMIZERS, & WEIGHT INIT
# ----------------------------------------------------------------
class NeuralNetwork:
    def __init__(
        self,
        ...
        """
        Parameters:
          input_size    : int, e.g., 784 for 28x28
          hidden_layers : list of ints, e.g., [128, 64]
          output_size   : int, number of classes (e.g. 10)
          optimizer     : str, one of {1,2,3,4,5,6} for [SGD, Momentum, Nesterov, RMSProp, Adam, Nadam]
          momentum      : momentum coeff for momentum-based methods
          rmsprop_beta  : decay rate for RMSProp
          adam_beta1    : beta1 for Adam/Nadam
          adam_beta2    : beta2 for Adam/Nadam
          epsilon       : small value to avoid div by 0
          weight_init   : 'random' or 'xavier'
          l2_lambda     : L2 regularization coeff
    ...
    def backward(self, layer_outputs, layer_inputs, targets):
        ...
        weight_grads = []
        bias_grads = []
        # For cross-entropy, gradient wrt final layer is (prediction - targets)
...
# ----------------------------------------------------------------
def train_sweep():
    run = wandb.init(project="CS24M046_DA6401_Assign1_Q7")
    config = wandb.config
    ...
    # Split off validation set (10% of training)
    ...
    test_preds = np.argmax(test_out[-1], axis=1)
    ...
    # -----------------------------
    # CONFUSION MATRIX (INT COUNTS)
    # -----------------------------
    cm = confusion_matrix(y_test, test_preds)
    plt.figure(figsize=(8, 6))
    sns.heatmap(
        cm,
        annot=True,    # put exact counts inside each cell
        fmt="d",       # integer format
        cmap="Blues",
        linewidths=1,  # draw lines between cells
        linecolor='black'
    )
...
if __name__ == "__main__":
    sweep_id = wandb.sweep(sweep_config, project="CS24M046_DA6401_Assign1_Q7")
    wandb.agent(sweep_id, train_sweep, project="CS24M046_DA6401_Assign1_Q7")
